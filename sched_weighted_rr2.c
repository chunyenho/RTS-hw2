...
static struct task_struct *pick_next_task_weighted_rr(struct rq *rq)
{
	...
	struct task_struct *exe_next;
        struct task_struct *next_select;
	...
	for(i=0;i<rq->weighted_rr.nr_running ;i++){
        	next = list_first_entry(&rq->weighted_rr.queue,struct task_struct, weighted_rr_list_item);
        	tempTime=next->weighted_time_slice;
		if(tempTime<exeTime){
			exeTime=tempTime;
			exe_next=next;
		//	exe_next->se.exec_start = rq->clock;
		}
		
		list_move_tail(&next->weighted_rr_list_item, &rq->weighted_rr.queue);
	//	next->se.exec_start = rq->clock;
	}
	next_select=list_first_entry(&rq->weighted_rr.queue,struct task_struct, weighted_rr_list_item);
	while(next_select->weighted_time_slice != exeTime){
		list_move_tail(&next_select->weighted_rr_list_item, &rq->weighted_rr.queue);
		next_select=list_first_entry(&rq->weighted_rr.queue,struct task_struct, weighted_rr_list_item);
	}
